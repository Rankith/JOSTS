...
def element_search(request):
    if request.session.get('disc_path') == 'tra':
        ...
        gr_cats = AcroBalance.objects.filter(event="GR").exclude(category='B').exclude(category='M').exclude(category='').order_by('category').values_list('category',flat=True).distinct()
        tr_cats = AcroBalance.objects.filter(event="TR").exclude(category='B').exclude(category='M').exclude(category='').order_by('category').values_list('category',flat=True).distinct()
        events=request.session.get('disc_events','V,UB,BB,FX').split(",")
        context = {
            'categories':cats,
        ...
        if data["Bases"][i] != None and data["Tops"][i] != None:
            ...
            if len(AcroInvalid.objects.filter((Q(base=base) | Q(base=None)) & (Q(top=top) | Q(top=None)) & (Q(invalid_for=pair_type) | Q(invalid_for='ALL')))) > 0:
                ...
                if data["Hold"][i]:
                    ...
                    if top.legs_together_bonus > 0:
                        score_dict["top_value"] = score_dict["top_value"] + " + " + str(top.legs_together_bonus)
                    total = base_value + top_value + w_bonus + first_bonus + top.legs_together_bonus
                    first_bonus += w_bonus    
                else:
...
def video_notes_builder(request):
    ...
    rules = RuleLink.objects.filter(event='',disc=request.session.get('disc',1)) | RuleLink.objects.filter(event__contains=event.upper(),disc=request.session.get('disc',1))
    events=request.session.get('disc_events','V,UB,BB,FX').split(",")
    context = {
        'elements': elements,
        'rules': rules,

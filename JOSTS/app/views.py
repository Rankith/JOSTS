...
def element_search(request):
    if request.session.get('disc_path') == 'tra':
        ...
        gr_cats = AcroBalance.objects.filter(event="GR").exclude(category='B').exclude(category='M').order_by('category').values_list('category',flat=True).distinct()
        tr_cats = AcroBalance.objects.filter(event="TR").exclude(category='B').exclude(category='M').order_by('category').values_list('category',flat=True).distinct()
        events=request.session.get('disc_events','V,UB,BB,FX').split(",")
        context = {
            'categories':cats,
            'gr_categories':'',
